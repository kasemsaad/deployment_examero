...
const DeleteQuestationModel = ({ idOfDeleteItem, fetchAllData, content }) => {
  const deleteOpenEmis = async (idOfDeleteItem) => {
    document.body.style.removeProperty("overflow");
    if (idOfDeleteItem) {
      try {
        await Api_website.delete(
          `/teachers/questions/${idOfDeleteItem}`
        );
        fetchAllData();
      } catch (error) {
  ...
  return (
    <>
      ...
      <div
        ...
        id="deleteElementModal_questationn"
        tabIndex="-1"
...
export default DeleteQuestationModel;
