...
function ExamPdf_dash() {
  ...
  return (
    <>
      ...
      <div className="container container-exame  rounded-4 py-3 my-5 shadow" dir="rtl" style={{ height: "40vh", overflowY: "auto", backgroundColor: "rgb(224, 224, 224)" }} >
            ...
            return (
              <div key={index} className="exam-question  container-exame"   >
                ...
                <div className=" draggable" draggable="true"><span style={{ textDecoration: "underline", fontWeight: "bold", fontSize: "24px" }}>{index + 1}) {item.addressQuestion} <span> </span>
                  <span>({item.markQuestion}) درجه</span><br /></span>
                </div>
                    ...
                    <div className="d-flex my-2 draggable" draggable="true">
                      {(markquestion.options).map((option, index) => (
                        <div className='' key={index} style={{ fontSize: "21.33px", padding: "0px 80px" }}>
                          ...
                          <span>) </span>
                          {option.option}
                        <span  style={{paddingRight:"0px", color:"green"}}> {option.is_correct===1?"الإجابة الصحيحة":""   } </span>
      ...
      <div
        ...
        style={{
          height: `${MAX_HEIGHT}px`, position: "relative", width:"1145px",
      ...
      >
        <div className='section1  pt-2 ' style={{ width: "100%", position: "relative", top: "0", right: "0" }}>
          ...
          <div className="exam-content" dir="rtl" >
            <div dir="rtl" align="center" className="exam-info" >
              <span style={{ fontWeight: "bold" }}>{headerData1.institution}</span><br />
              مديرية  <span style={{ fontWeight: "bold" }}>{headerData1.directorate}</span><br />
              مدرسة  <span style={{ fontWeight: "bold" }}>{headerData1.school}</span><br />
              {headerData1.examName}    {parseInt(headerData2.semester) === 1 ? <span style={{ fontWeight: "bold" }}>الفصل الدرسي الأول</span> : parseInt(headerData2.semester) === "2" ? <span style={{ fontWeight: "bold" }}>الفصل الدرسي الثاني</span> : ""} <span style={{ fontWeight: "bold" }}> {headerData2.curriculum}</span>
            </div>
            <div dir="rtl" className="exam-details d-flex justify-content-between">
              <div dir="rtl" align="start">
                الصف و الشعبة : <span style={{ fontWeight: "bold" }}>{headerData2.examName}</span><br />
                المبحث: <span style={{ fontWeight: "bold" }}>{headerData3.subjectname}</span><br />
                {headerData2.directorate}<span style={{ fontWeight: "bold" }}> .....................................</span>
              </div>
              <div dir="rtl" align="start">
                التاريخ: <span style={{ fontWeight: "bold" }}>{headerData2.dayAndDate}</span><br />
                مدة الامتحان :<span style={{ fontWeight: "bold" }}> {headerData2.examDuration}  دقيقة فقط </span><br />
              </div>
            </div>
          </div>

          <div dir="rtl" className="exam-note  pb-3" hidden={!headerData3.showApperanceNotice}>

            <span style={{ fontSize: "21.33px", fontWeight: "bold" }}>
              ملحوظة مهمة : أجب عن الأسئلة الآتية جميعها وعددها ({headerData3.questionCount}) ، علماً أن عدد صفحات الامتحان ({count})</span>
          </div>
        </div>
      ...
      {divs.map((div) => (
        <div key={div.id} dir="rtl">
          <i
            ...
            className="fas fa-xmark mt-3 px-2"
            style={{ marginRight: "170px", padding: "6px", borderRadius: '50%', backgroundColor: "#4941A6", color: "white" }} // Optional: adjust the margin as needed
          >
          </i>
          <div
          ...
          style={{ width:"1145px", backgroundColor: "#4941A6", color: "white" }}>إضافه صفحه</button>
      </div>
      ...
      <div
        ...
        style={{ height: `${MAX_HEIGHT}px`, position: "relative", width:"1145px" }}

      >
        <div
          ref={containerRef}
          className="container container-exame myDiv"
          style={{ height: `${MAX_HEIGHT - 170}px`, position: "relative" , width:"1145px"}}
          onDrop={handleDrop}
          onDragOver={handleDragOver}
          dir='rtl'
          align="start"
        ...
        <div className="exam-footer   m-0 p-0  " style={{ width: "100%", height: "120px", position: "relative", bottom: "0", right: "0", borderTop: "1px solid" }} >
          ...
          <div dir='rtl' className="exam-teacher">
      ...
      <div dir="rtl" className="p-0 ">
        <div className='d-flex align-items-center justify-content-center p-5'>
          <Link hidden={true} to={`/dashboard/Put5/${parseInt(headerData3.questionCount) + 1}`}>
            <button className="btn_putting_exam2_after text-white py-0 m-0 mt-3 mx-2 p-5">
              Previous
            </button>
          </Link>
          <Link to={"/ExamPdfArabic"}>
            <button className="btn_putting_exam2_after text-white py-0 m-0 mt-3 mx-2 p-5" style={{backgroundColor:"#4941A6"}} >صفحه الاسئلة</button>
          </Link>
          <Link>
            <button className="btn_putting_exam2_after text-white py-0 m-0 mt-3 mx-2 p-5" style={{backgroundColor:"#4941A6"}} onClick={downloadPdf}>تحميل</button>
          </Link>
          <Link>
            <button className="btn_putting_exam2_after text-white py-0 m-0 mt-3 mx-2 p-5" style={{backgroundColor:"#4941A6"}} data-bs-toggle="modal" data-bs-target="#logout">حفظ</button>
          </Link>
        </div>
      </div>
