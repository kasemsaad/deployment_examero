...
function Put3(props) {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        const response = await Api_Dashboard.get('/groups/selection');
    ...
    const fetchSubjects = async () => {
      try {
        const response = await Api_Dashboard.get('/subjects/selection/groupId');
  ...
  let jsonArray = [localStorage.getItem("doc")];
...
let techid =  parseInt (JSON.parse(parsedArray[0][0]).teacher_id);
console.log(techid);


  const plans = () => {
    Api_Dashboard.get(`/plans/${techid}/teacher`)
      .then((response) => {
  ...
  const handleChangeGroup = async (eventKey) => {
    ...
    if (selectedGroup) {
      ...
      try {
        const response = await Api_Dashboard.get(`/subjects/selection/${selectedGroup.id}`);
        setAllSubject(response.data.data);
      } catch (error) {
        console.error("Error fetching subjects data:", error);
      }
